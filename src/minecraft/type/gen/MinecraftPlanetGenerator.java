...
public class MinecraftPlanetGenerator extends PlanetGenerator{
    ...
    Block[][] arr =
            {
                    {stone,stone,grass,water_still,stone_andesite,gravel,lava},
                    {gravel,stone,grass,water_still,stone,gravel,lava},
                    {water_still,stone_granite,grass,water_still,stone,lava,lava},
                    {gravel,stone,grass,water_still,stone_diorite,gravel,lava},
            };

    ObjectMap<Block, Block> dec = ObjectMap.of(
            MinecraftBlocks.water_still,stone,gravel,stone_andesite,stone_diorite,stone_granite
    );
    ...
    protected void generate(){
                        ...
                        for(int y = -rad; y <= rad; y++){
                            ...
                            if(Structs.inBounds(wx, wy, width, height) && Mathf.within(x, y, rad)){
                                ...
                                if(Mathf.within(x, y, rad - 1) && !other.floor().isLiquid){
                                    ...
                                    other.setFloor((Floor)(floor == sand || floor == clay ? Blocks.sandWater : Blocks.darksandTaintedWater));
                                }
                            }
                        }
                    }
        ...
        pass((x, y) -> {
            ...
            if(value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)){
                ...
                boolean gravel = floor != sand && floor != clay;
                //do not place rivers on ice, they're frozen
                //ignore pre-existing liquids
                if(!(floor == Blocks.ice || floor == Blocks.iceSnow || floor == snow || floor.asFloor().isLiquid)){
                    floor = gravel ?
        ...
        if(naval){
            ...
            pass((x, y) -> {
                if(floor.asFloor().isLiquid && !floor.asFloor().isDeep() && !floor.asFloor().shallow){
                    ...
                    floor = floor == water_still ? Blocks.deepwater : Blocks.taintedWater;
                }
            });
        }
        ...
        pass((x, y) -> {
            ...
            }else if(genLakes && floor != sand && floor != clay && floor.asFloor().hasSurface()){
                float noise = noise(x + 782, y, 5, 0.75f, 260f, 1f);
                if(noise > 0.67f && !roomseq.contains(e -> Mathf.within(x, y, e.x, e.y, 14))){
                    if(noise > 0.72f){
                        floor = noise > 0.78f ? water_still : (floor == Blocks.sand ? Blocks.sandWater : Blocks.darksandTaintedWater);
                    }else{
